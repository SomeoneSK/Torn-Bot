	...
	obj.check = async function(stocks) {
		...
		if ( difference >= 0  ) {
			...
			if ( chan !== undefined && mention !== undefined ) {
				chan.send(mention + ", **" + this.alert.stock + "**'s **" +  this.alert.property.replace("_", " ") + "** is now " + this.alert.higher_or_lower + " than " + General_functions.format_number(this.alert.value) + " - it's **" + General_functions.format_number(stocks["stocks"][index][this.alert.property]) + "** !")
			}
