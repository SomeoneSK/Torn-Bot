...
async function pagination( the_messages, interaction, default_page = 1 ) {
  ...
  async function reply(message, add_buttons){
	...
	async function add_the_buttons(message) {
		let buttons = []
		let next_page_button = await Components_functions.button(interaction, "next_page", "Next", "SECONDARY", next_page)
		let previous_page_button = await Components_functions.button(interaction, "previous_page", "Previous", "SECONDARY", previous_page)
